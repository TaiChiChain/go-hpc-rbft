...
type rbftImpl struct {
	...
	recvChan chan consensusEventWrapper // channel to receive ordered consensus messages and local events
...
func newRBFT(cpChan chan *types.ServiceState, confC chan uint64, c Config) (*rbftImpl, error) {
	...
	recvC := make(chan consensusEventWrapper, 1)
	rbft := &rbftImpl{
		K:                    c.K,
		epoch:                c.EpochInit,
		config:               c,
...
func (rbft *rbftImpl) start() error {
	...
	rbft.postMsg(context.Background(), initRecoveryEvent)
...
func (rbft *rbftImpl) reportStateUpdated(state *types.ServiceState) {
	...
	go rbft.postMsg(context.Background(), event)
}
...
func (rbft *rbftImpl) postRequests(requests *pb.RequestSet) {
	...
	rbft.postMsg(context.Background(), requests)
}

// postBatches informs RBFT batch event which is usually generated by request pool.
func (rbft *rbftImpl) postBatches(ctx context.Context, batches []*txpool.RequestHashBatch) {
	for _, batch := range batches {
		_ = rbft.recvRequestBatch(ctx, batch)
	}
}
...
func (rbft *rbftImpl) postMsg(ctx context.Context, msg interface{}) {
	rbft.recvChan <- consensusEventWrapper{
		ctx:   ctx,
		event: msg,
	}
}
	...
	for {
		...
		case w := <-rbft.recvChan:
			...
			for {
				...
				next = rbft.processEvent(w.ctx, next)
				if next == nil {
					break
				}
			}
...
func (rbft *rbftImpl) processEvent(ctx context.Context, ee consensusEvent) consensusEvent {
	switch e := ee.(type) {
	...
	case *pb.RequestSet:
		...
		rbft.processReqSetEvent(ctx, e)

		return nil

	case *LocalEvent:
		return rbft.dispatchLocalEvent(ctx, e)

	case *pb.ConsensusMessage:
		return rbft.consensusMessageFilter(ctx, e)
...
func (rbft *rbftImpl) dispatchCoreRbftMsg(ctx context.Context, e consensusEvent) consensusEvent {
	switch et := e.(type) {
	case *pb.NullRequest:
		return rbft.processNullRequest(ctx, et)
	case *pb.PrePrepare:
	...
	case *pb.FetchMissingRequests:
		return rbft.recvFetchMissingTxs(ctx, et)
	case *pb.SendMissingRequests:
		return rbft.recvSendMissingTxs(ctx, et)
	case *pb.SignedCheckpoint:
		return rbft.recvCheckpoint(ctx, et, false)
	}
	return nil
}
...
func (rbft *rbftImpl) processNullRequest(ctx context.Context, msg *pb.NullRequest) consensusEvent {
...
func (rbft *rbftImpl) sendNullRequest(ctx context.Context) {
...
func (rbft *rbftImpl) processReqSetEvent(ctx context.Context, req *pb.RequestSet) consensusEvent {
	...
	if rbft.atomicIn(InConfChange) {
		for _, tx := range req.Requests {
			if hpcCommonTypes.IsConfigTx(tx) {
				rbft.logger.Debugf("Replica %d is in config change, add ctx %s into txpool", rbft.peerPool.ID, hpcCommonTypes.GetHash(tx).String())
			}
		}
	}
...
func (rbft *rbftImpl) recvRequestBatch(ctx context.Context, reqBatch *txpool.RequestHashBatch) error {
	...
	if rbft.isPrimary(rbft.peerPool.ID) && rbft.isNormal() && !rbft.atomicIn(InConfChange) {
		...
		rbft.maybeSendPrePrepare(ctx, batch, false)
	} else {
...
func (rbft *rbftImpl) sendPrePrepare(ctx context.Context, seqNo uint64, digest string, reqBatch *pb.RequestBatch) {
	...
	ctx, span := rbft.tracer.Start(ctx, "sendPrePrepare")
...
func (rbft *rbftImpl) recvCommit(ctx context.Context, commit *pb.Commit) error {
	ctx, span := rbft.tracer.Start(ctx, "recvCommit")
	defer span.End()
...
func (rbft *rbftImpl) recvFetchMissingTxs(ctx context.Context, fetch *pb.FetchMissingRequests) error {
...
func (rbft *rbftImpl) recvSendMissingTxs(ctx context.Context, re *pb.SendMissingRequests) consensusEvent {
...
func (rbft *rbftImpl) commitPendingBlocks(ctx context.Context) {
	...
	for hasTxToExec := true; hasTxToExec; {
		if find, idx, cert := rbft.findNextCommitBatch(); find {
			...
			rbft.afterCommitBlock(ctx, idx, isConfig)
		} else {
			hasTxToExec = false
		}
	}
...
func (rbft *rbftImpl) afterCommitBlock(ctx context.Context, idx msgID, isConfig bool) {
	...
	if isConfig {
		...
		if state.MetaState.Height == rbft.exec.lastExec {
			...
			rbft.checkpoint(ctx, state, isConfig)
		} else {
...
func (rbft *rbftImpl) checkpoint(ctx context.Context, state *types.ServiceState, isConfig bool) {
	...
	rbft.peerPool.broadcast(ctx, consensusMsg)
	rbft.recvCheckpoint(ctx, signedCheckpoint, true)
}

// recvCheckpoint processes logic after receive checkpoint.
func (rbft *rbftImpl) recvCheckpoint(ctx context.Context, signedCheckpoint *pb.SignedCheckpoint, local bool) consensusEvent {
	...
	if signedCheckpoint.Checkpoint.Reconfiguration() {
		return rbft.finishConfigCheckpoint(ctx, checkpointHeight, checkpointDigest, matchingCheckpoints)
	}

	return rbft.finishNormalCheckpoint(ctx, checkpointHeight, checkpointDigest, matchingCheckpoints)
}

func (rbft *rbftImpl) finishConfigCheckpoint(ctx context.Context, checkpointHeight uint64, checkpointDigest string,
...
func (rbft *rbftImpl) finishNormalCheckpoint(ctx context.Context, checkpointHeight uint64, checkpointDigest string, matchingCheckpoints []*pb.SignedCheckpoint) consensusEvent {
	rbft.stopFetchCheckpointTimer()
...
func (rbft *rbftImpl) weakCheckpointSetOutOfRange(signedCheckpoint *pb.SignedCheckpoint) bool {
	...
	if checkpointHeight < H {
		...
		if len(rbft.storeMgr.higherCheckpoints) >= rbft.oneCorrectQuorum() {
			...
			if highestWeakCertMeta.Height > H {
...
func (rbft *rbftImpl) recvStateUpdatedEvent(ctx context.Context, ss *types.ServiceState) consensusEvent {
	...
	rbft.afterStateUpdate(ctx)
	return nil
}

// afterStateUpdate processes logic after state update
func (rbft *rbftImpl) afterStateUpdate(ctx context.Context) {

	if rbft.isPrimary(rbft.peerPool.ID) {
		rbft.logger.Debugf("Replica %d is primary, not prepare after stateUpdate", rbft.peerPool.ID)
		return
	}

	rbft.logger.Debugf("Replica %d try to prepare after stateUpdate", rbft.peerPool.ID)

	for idx, cert := range rbft.storeMgr.certStore {
		// If this node is not primary, it would prepare pending transactions.
		if rbft.prePrepared(idx.v, idx.n, idx.d) && !cert.sentPrepare {
			rbft.logger.Debugf("Replica %d try to prepare batch %s", rbft.peerPool.ID, idx.d)
			_ = rbft.findNextPrepareBatch(ctx, idx.v, idx.n, idx.d)
		}
	}
}
