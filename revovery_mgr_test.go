...
func TestRecovery_ClusterInitRecovery(t *testing.T) {
	...
	rbfts[1].processEvent(context.Background(), notificationSet[0])
	notificationSet[1] = nodes[1].broadcastMessageCache
	assert.Equal(t, pb.Type_NOTIFICATION, notificationSet[1].Type)

	rbfts[2].processEvent(context.Background(), notificationSet[0])
	notificationSet[2] = nodes[2].broadcastMessageCache
	assert.Equal(t, pb.Type_NOTIFICATION, notificationSet[2].Type)

	rbfts[3].processEvent(context.Background(), notificationSet[1])
	rbfts[3].processEvent(context.Background(), notificationSet[2])
	...
	for index := range rbfts {
		for j := range notificationSet {
			done := rbfts[index].processEvent(context.Background(), notificationSet[j])
			if done != nil {
	...
	for index := range rbfts {
		rbfts[index].processEvent(context.Background(), doneNotificationQuorum)
	}
	...
	for index := range rbfts {
		...
		done := rbfts[index].processEvent(context.Background(), nv)
		assert.Equal(t, doneRecovery, done)
	}

	fetchPQCSet := make([]*pb.ConsensusMessage, 4)
	for index := range rbfts {
		rbfts[index].processEvent(context.Background(), doneRecovery)
...
func TestRecovery_ReplicaRecovery(t *testing.T) {
	...
	for index := range rbfts {
		...
		rbfts[index].processEvent(context.Background(), notificationNode4)
	...
	for index := range notificationRsp {
		...
		rbfts[3].processEvent(context.Background(), notificationRsp[index])
	}
	...
	rbfts[0].processEvent(context.Background(), fetchMissingReqBatch)
	...
	recoveryDone := rbfts[3].processEvent(context.Background(), sendReqBatch)
	...
	rbfts[3].processEvent(context.Background(), recoveryDone)
	...
	for index := range rbfts {
		...
		rbfts[index].processEvent(context.Background(), node4FetchPQC)
	...
	for index := range returnRecoveryPQC {
		...
		rbfts[3].processEvent(context.Background(), returnRecoveryPQC[index])
	}
...
func TestRecovery_PrimaryRecovery(t *testing.T) {
	...
	for index := range rbfts {
		...
		rbfts[index].processEvent(context.Background(), notificationNode1)
		notificationRsp[index] = nodes[index].unicastMessageCache
		assert.Equal(t, pb.Type_NOTIFICATION_RESPONSE, notificationRsp[index].Type)
	}
	...
	for index := range notificationRsp {
		...
		rbfts[0].processEvent(context.Background(), notificationRsp[index])
	}
...
func TestRecovery_NormalCheckpointFailing_Recovery(t *testing.T) {
	...
	recoveryDone := rbfts[3].processEvent(context.Background(), event)
...
func TestRecovery_EpochCheckpointFailing_Recovery(t *testing.T) {
	...
	for index := range rbfts {
		...
		rbfts[index].processEvent(context.Background(), preNotificationNode4)
	...
	for index := range preNotificationRsp {
		...
		rbfts[3].processEvent(context.Background(), preNotificationRsp[index])
	}
	...
	rbfts[3].processEvent(context.Background(), ev1)
...
func TestRecovery_SyncStateToStateUpdate(t *testing.T) {
	...
	rbfts[3].processEvent(context.Background(), syncEvent)
	...
	for index := range rbfts {
		rbfts[index].processEvent(context.Background(), node4SyncStateReq)
	...
	for index := range syncStateResponse {
		rbfts[3].processEvent(context.Background(), syncStateResponse[index])
	}
	...
	rbfts[3].processEvent(context.Background(), msg)
...
func TestRecovery_PrimarySyncStateToRecovery(t *testing.T) {
	...
	rbfts[0].processEvent(context.Background(), syncEvent)
	...
	for index := range syncStateResponse {
		rbfts[0].processEvent(context.Background(), syncStateResponse[index])
	}
