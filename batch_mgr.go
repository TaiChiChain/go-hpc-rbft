...
func newBatchManager[T any, Constraint consensus.TXConstraint[T]](requestPool mempool.MemPool[T, Constraint], c Config) *batchManager[T, Constraint] {
...
func (rbft *rbftImpl[T, Constraint]) startCheckPoolRemoveTimer() {
	...
	rbft.logger.Debugf("Replica %d started the check pool remove timer, need to remove invalid tx in mempool after %v", rbft.peerMgr.selfID, rbft.timerMgr.getTimeoutValue(checkPoolRemoveTimer))
}
...
func (rbft *rbftImpl[T, Constraint]) restartCheckPoolRemoveTimer() {
	...
	rbft.timerMgr.startTimer(checkPoolRemoveTimer, localEvent)
	rbft.logger.Debugf("Replica %d restarted the check pool remove timer, need to remove invalid tx in mempool after %v", rbft.peerMgr.selfID, rbft.timerMgr.getTimeoutValue(checkPoolRemoveTimer))
}
